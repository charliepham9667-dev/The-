...
/**
 * Safely get current position with a short timeout. Never throws.
 * If geolocation fails (permission denied, timeout, position unavailable, etc.),
 * returns null so clock-in can still proceed without location data.
 */
async function safeGetPosition(): Promise<{ latitude: number; longitude: number } | null> {
  try {
    const position = await Promise.race([
      getCurrentPosition({ enableHighAccuracy: false, timeout: 5000, maximumAge: 30000 }),
    ...
    // Location unavailable, permission denied, timeout, etc. — proceed without it
    return null;
  }
}
...
export function useClockIn() {
  ...
  return useMutation({
    mutationFn: async ({ shiftId }: { shiftId?: string } = {}) => {
      ...
      // Get current location (never blocks clock-in — proceeds without location if unavailable)
      let latitude: number | undefined;
      let longitude: number | undefined;
      let isWithinGeofence = false;
      let distanceFromVenue: number | undefined;

      const position = await safeGetPosition();
      if (position) {
...
export function useClockOut() {
  ...
  return useMutation({
    mutationFn: async ({ shiftId }: { shiftId?: string } = {}) => {
      ...
      // Get current location (never blocks clock-out — proceeds without location if unavailable)
      let latitude: number | undefined;
      let longitude: number | undefined;
      let isWithinGeofence = false;
      let distanceFromVenue: number | undefined;
