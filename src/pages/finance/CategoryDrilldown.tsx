...
const colors = ['bg-[#ff6b35]', 'bg-blue-500', 'bg-emerald-500', 'bg-purple-500', 'bg-yellow-500'];
...
export function CategoryDrilldown() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-semibold text-white">Category Drill-down</h1>
        <p className="text-sm text-slate-400 mt-1">Revenue breakdown by category - January 2026</p>
      </div>
      ...
      <div className="rounded-xl border border-[#374151] bg-[#1a1f2e] p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-3xl font-bold text-white">{formatVND(totalRevenue)}</p>
          </div>
        ...
        <div className="flex flex-wrap gap-4 mt-4">
          {categories.map((category, i) => (
            <div key={category.id} className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400">{category.name} ({category.percentage}%)</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4">
        {categories.map((category, i) => (
          <div key={category.id} className="rounded-xl border border-[#374151] bg-[#1a1f2e] overflow-hidden">
            <button
              onClick={() => setExpandedCategory(expandedCategory === category.id ? null : category.id)}
              className="flex items-center justify-between w-full p-4 hover:bg-[#0f1419] transition-colors"
            >
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs text-slate-400">{category.subcategories.length} subcategories</p>
                </div>
              </div>
              <div className="flex items-center gap-6">
                <div className="text-right">
                  <p className="text-sm font-semibold text-white">{formatVND(category.revenue)}</p>
                  <p className="text-xs text-slate-400">{category.percentage}% of total</p>
                </div>
                <div className={`flex items-center gap-1 ${category.change >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                ...
                <ChevronRight className={`h-5 w-5 text-slate-400 transition-transform ${expandedCategory === category.id ? 'rotate-90' : ''}`} />
              </div>
            </button>
            
            {expandedCategory === category.id && (
              <div className="border-t border-[#374151] p-4 bg-[#0f1419]">
                <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                  {category.subcategories.map((sub) => (
                    <div key={sub.name} className="rounded-lg border border-[#374151] bg-[#1a1f2e] p-3">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm text-white">{sub.name}</span>
                        <span className="text-xs text-slate-400">{sub.percentage}%</span>
                      </div>
                      <p className="text-lg font-semibold text-white">{formatVND(sub.revenue)}</p>
                      <div className="h-1.5 rounded-full bg-[#374151] mt-2">
