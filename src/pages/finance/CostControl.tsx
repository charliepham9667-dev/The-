...
const statusConfig = {
  under: { icon: CheckCircle, color: 'text-emerald-400', bg: 'bg-emerald-500/20', label: 'Under Budget' },
  'on-track': { icon: Target, color: 'text-blue-400', bg: 'bg-blue-500/20', label: 'On Track' },
  over: { icon: AlertTriangle, color: 'text-red-400', bg: 'bg-red-500/20', label: 'Over Budget' },
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-white">Cost Control</h1>
          <p className="text-sm text-slate-400 mt-1">Budget tracking and optimization</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-xl border border-[#374151] bg-[#1a1f2e] p-4">
          ...
          <p className="text-xl font-bold text-white">{formatVND(totalActual)}</p>
          <p className="text-xs text-slate-400 mt-1">Budget: {formatVND(totalBudget)}</p>
        </div>
        <div className={`rounded-xl border p-4 ${variance >= 0 ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-red-500/30 bg-red-500/10'}`}>
          <div className={`flex items-center gap-2 mb-2 ${variance >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
          ...
          <p className="text-xs text-slate-400 mt-1">{variance >= 0 ? 'Under budget' : 'Over budget'}</p>
        </div>
        <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-4">
          ...
          <p className="text-xl font-bold text-red-400">{overBudgetCount}</p>
          <p className="text-xs text-slate-400 mt-1">categories need attention</p>
        </div>
        <div className="rounded-xl border border-[#374151] bg-[#1a1f2e] p-4">
          <div className="flex items-center gap-2 text-slate-400 mb-2">
          ...
          <p className="text-xl font-bold text-white">{((totalActual / 794000000) * 100).toFixed(1)}%</p>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
                ...
                <div key={category.id} className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      <span className="text-slate-400">{formatVND(category.actual)}</span>
                      <span className="text-slate-500">/</span>
                      <span className="text-white">{formatVND(category.budget)}</span>
                    </div>
                  </div>
                  <div className="h-2 rounded-full bg-[#374151]">
                    <div
                      className={`h-2 rounded-full transition-all ${
                        ...
                        category.status === 'on-track' ? 'bg-blue-500' : 'bg-emerald-500'
                      }`}
        ...
        <div className="rounded-xl border border-[#374151] bg-[#1a1f2e] p-6">
          <h2 className="text-lg font-semibold text-white mb-4">Cost Saving Opportunities</h2>
          <div className="space-y-3">
            {costSavingTips.map((tip) => (
              <div key={tip.id} className="rounded-lg bg-[#0f1419] p-4">
                <p className="text-sm text-slate-300 mb-2">{tip.tip}</p>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-emerald-400">Potential savings: {tip.savings}</span>
                  <button className="text-xs text-[#ff6b35] hover:underline">Apply</button>
                </div>
              </div>
            ))}
          </div>
