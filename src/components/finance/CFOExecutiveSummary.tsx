...
function calculateBHI(
  ...
  const weakestPillar = [
    ...
    { name: 'operational efficiency', score: efficiencyScore },
    { name: 'revenue diversification', score: resilienceScore },
  ].reduce((min, p) => p.score < min.score ? p : min);
...
/**
 * Legacy function for backward compatibility - wraps BHI calculation
 */
function classifyBusinessHealth(
  current: PnlMonthly,
  budget: PnlMonthly | null
): { status: HealthStatus; score: number; reasoning: string } {
  // Use BHI calculation with null previous month
  const bhi = calculateBHI(current, null, budget);
  
  return {
    status: bhi.rating,
    score: bhi.score,
    reasoning: bhi.executiveSummary.split('.')[0] || 'Analysis complete'
  };
}
...
/**
 * Generate executive summary text (3-5 sentences)
 */
function generateExecutiveSummary(
  current: PnlMonthly,
  previous: PnlMonthly | null,
  health: { status: HealthStatus; score: number; reasoning: string },
  momAnalysis: MoMAnalysis
): string {
  const monthName = getMonthName(current.month);
  const ebitMargin = current.grossSales > 0 ? (current.ebit / current.grossSales) * 100 : 0;
  ...
  // Sentence 1: Health classification
  summary += `Business health for ${monthName} is classified as **${health.status}** `;
  if (health.status === 'Excellent' || health.status === 'Good') {
    summary += `with solid fundamentals driven by ${health.reasoning}. `;
  } else if (health.status === 'Stable') {
    summary += `with adequate performance despite ${health.reasoning}. `;
  } else {
    summary += `requiring attention due to ${health.reasoning}. `;
  }

  // Sentence 2: Revenue and margin movements
  if (previous) {
    if (momAnalysis.revenueChangePercent > 5) {
      summary += `Revenue advanced ${formatPercent(momAnalysis.revenueChangePercent)} month-over-month to ${formatCurrency(current.grossSales)}, `;
    } else if (momAnalysis.revenueChangePercent < -5) {
      summary += `Revenue contracted ${formatPercent(Math.abs(momAnalysis.revenueChangePercent))} month-over-month to ${formatCurrency(current.grossSales)}, `;
    } else {
      summary += `Revenue remained stable at ${formatCurrency(current.grossSales)}, `;
    }
    
    if (momAnalysis.grossMarginChange > 1) {
      summary += `with gross margin expanding ${momAnalysis.grossMarginChange.toFixed(1)}%. `;
    } else if (momAnalysis.grossMarginChange < -1) {
      summary += `though gross margin compressed ${Math.abs(momAnalysis.grossMarginChange).toFixed(1)}%. `;
    } else {
      summary += `with margins holding steady. `;
    }
  } else {
    summary += `Revenue reached ${formatCurrency(current.grossSales)} with EBIT margin of ${ebitMargin.toFixed(1)}%. `;
  }

  // Sentence 3: Operational efficiency
  if (laborRatio < 18 && cogsRatio < 28) {
    summary += `Operational efficiency remains strong with labor and COGS ratios within target parameters. `;
  } else if (laborRatio > 20 || cogsRatio > 30) {
    summary += `Operational efficiency requires attention with ${laborRatio > 20 ? 'labor' : 'COGS'} costs elevated above optimal thresholds. `;
  } else {
    summary += `Operational metrics are acceptable though opportunities exist for tightening cost controls. `;
  }

  // Sentence 4: Forward outlook
  if (current.ebit > 0 && momAnalysis.isStructural && momAnalysis.revenueChangePercent > 0) {
    summary += `Forward outlook is favorable with structural growth momentum supporting continued margin expansion.`;
  } else if (current.ebit > 0) {
    summary += `Near-term outlook is cautiously optimistic, contingent on sustaining current cost discipline.`;
  } else {
    summary += `Immediate focus must be on restoring profitability before pursuing growth initiatives.`;
  }
...
export function CFOExecutiveSummary({ 
  ...
  const bhi = useMemo(
    () => calculateBHI(currentMonth, previousMonth, budgetMonth),
    [currentMonth, previousMonth, budgetMonth]
  );
  
  // Legacy health object for backward compatibility
  const health = useMemo(
    () => ({ status: bhi.rating, score: bhi.score, reasoning: bhi.executiveSummary.split('.')[0] }),
    [bhi]
  );
  ...
  const drivers = useMemo(
    () => identifyKeyDrivers(currentMonth, previousMonth, budgetMonth, momAnalysis),
    [currentMonth, previousMonth, budgetMonth, momAnalysis]
  );
  ...
  const executiveSummary = useMemo(
    () => generateExecutiveSummary(currentMonth, previousMonth, health, momAnalysis),
    [currentMonth, previousMonth, health, momAnalysis]
  );
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      ...
      <div className="p-6 space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-4">
            ...
            <p className="text-[10px] text-slate-500 uppercase tracking-widest">2. Month-to-Month Financial Analysis</p>
          </div>
          ...
          {previousMonth ? (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={idx} className="rounded-xl border border-[#2a3441] bg-[#151922] p-4">
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest mb-2">{metric.label}</p>
                    <div className="flex items-center gap-2">
                      <span className={`text-xl font-bold ${metric.isPositive ? 'text-emerald-400' : 'text-red-400'}`}>
                        {metric.value}
                      </span>
                      {metric.isPositive ? (
              ...
              {momAnalysis.categoryChanges.length > 0 && (
                <div className="grid grid-cols-4 md:grid-cols-8 gap-2">
                  {momAnalysis.categoryChanges.slice(0, 8).map(cat => {
                    ...
                    return (
                      <div key={cat.name} className="rounded-lg border border-[#2a3441] bg-[#151922] p-3">
                        <div className="flex items-center gap-1.5 mb-1">
                          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: color }} />
                          <span className="text-[10px] text-slate-400 uppercase tracking-wide">{cat.name}</span>
                        </div>
                        <span className={`text-sm font-bold ${cat.change >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            ...
            <div className="rounded-xl border border-[#2a3441] bg-[#151922] p-4">
              <p className="text-sm text-slate-500 italic">No prior month data available for comparison.</p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {/* Key Drivers */}
          <div className="rounded-xl border border-[#2a3441] bg-[#151922] p-5">
            <div className="flex items-center gap-2 mb-4">
              <Target className="h-4 w-4 text-emerald-400" />
              <p className="text-[10px] text-slate-500 uppercase tracking-widest">3. Key Drivers</p>
            </div>
            <div className="space-y-3">
              {drivers.positive.length > 0 ? drivers.positive.slice(0, 3).map((driver, i) => (
                <div key={i} className="rounded-lg bg-emerald-500/5 border border-emerald-500/20 p-3">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-sm font-medium text-emerald-400">{driver.title}</p>
                    {driver.metric && (
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed">{driver.description}</p>
                </div>
              )) : (
                <p className="text-sm text-slate-500 italic">No significant positive drivers identified.</p>
              )}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-[#2a3441] bg-[#151922] p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <p className="text-[10px] text-slate-500 uppercase tracking-widest">4. Strategic Implications</p>
            </div>
            <div className="space-y-3">
              {strategicActions.slice(0, 2).map((action, i) => (
                <div key={i} className="rounded-lg bg-[#1a1f2e] border-l-2 border-blue-500 p-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded uppercase tracking-wide font-medium ${
                      action.category === 'cost' ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400'
                    }`}>
                      {action.category}
                    </span>
                  </div>
                  <p className="text-sm text-slate-300 font-medium mb-1">{action.action}</p>
      ...
      {/* Footer */}
      <div className="border-t border-[#2a3441] px-6 py-3 bg-[#0d1117] flex items-center justify-between">
        <p className="text-[10px] text-slate-600 uppercase tracking-widest">Proprietary Information — Distribution Prohibited</p>
        <div className="flex items-center gap-4">
          <span className="text-[10px] text-slate-600 uppercase tracking-widest">The Roof Command Center</span>
          <span className="text-[10px] text-slate-600 uppercase tracking-widest">Page 01 of 01</span>
        </div>
      </div>
    </div>
  );
