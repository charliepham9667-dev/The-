...
function getHealthStatus(margin: number): { label: string; color: string; bgColor: string } {
  if (margin >= 25) return { label: 'HEALTHY', color: '#22C55E', bgColor: 'bg-emerald-500/10' };
  if (margin >= 15) return { label: 'WARNING', color: '#EAB308', bgColor: 'bg-yellow-500/10' };
  return { label: 'CRITICAL', color: '#EF4444', bgColor: 'bg-red-500/10' };
}
...
export function ProfitMarginGauge({ data, previousData, className = '' }: ProfitMarginGaugeProps) {
  if (!data) {
    return (
      <div className={`rounded-xl border border-[#374151] bg-[#1a1f2e] p-4 ${className}`}>
        <h3 className="text-sm font-medium text-white mb-4">Net Profit Margin</h3>
  ...
  return (
    <div className={`rounded-xl border border-[#374151] bg-[#1a1f2e] p-4 ${className}`}>
      <h3 className="text-sm font-medium text-white mb-2">Net Profit Margin</h3>
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="relative" style={{ width: size, height: size }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-3xl font-bold text-white">{margin.toFixed(0)}%</span>
            <span 
        ...
        {marginChange !== null && (
          <div className={`flex items-center gap-1 mt-3 text-sm ${
            marginChange >= 0 ? 'text-emerald-400' : 'text-red-400'
          }`}>
            {marginChange >= 0 ? (
